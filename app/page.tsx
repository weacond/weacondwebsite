...
export default function Home() {
  ...
  // 直接在代码里定义数据，不再通过外部 import，防止 Vercel 找不到文件报错
  const booksData = [
    {
      "id": 1,
      "cluster": "集群一：财富底层逻辑",
      ...
      {view === 'home' ? (
        ...
        <div className="py-16 px-6 max-w-6xl mx-auto text-center">
          ...
          <div className="flex overflow-x-auto gap-8 pb-10">
            {booksData.map((book) => (
              <div key={book.id} onClick={() => setSelectedBook(book)} className="min-w-[300px] bg-white border rounded-2xl p-8 cursor-pointer hover:shadow-xl transition-all">
                <div className="h-40 bg-gray-100 rounded-xl mb-6 flex items-center justify-center font-bold text-gray-300">COVER</div>
                <h3 className="text-xl font-bold mb-2">{book.title_cn}</h3>
                <p className="text-sm text-gray-400 italic mb-4">{book.title_en}</p>
                <div className="text-blue-600 font-bold">查看简介并阅读 →</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {selectedBook && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-3xl max-w-lg w-full p-10 relative">
            ...
            <p className="text-gray-600 mb-8 bg-gray-50 p-4 rounded-xl">{selectedBook.positioning}</p>
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => window.open(selectedBook.links.dual, '_blank')} className="py-4 bg-black text-white rounded-xl font-bold">双语对照 (DUAL MODE)</button>
            </div>
          </div>
        </div>
      )}
