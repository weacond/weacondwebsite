...
export default function Ebooks() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <Navbar onLangChange={setLang} currentLang={lang} />

      <div className="py-20 px-6 max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold mb-8">{lang === 'zh' ? '电子书' : 'Ebooks'}</h1>

        <div className="mb-8 flex items-center space-x-4">
          <label>{lang === 'zh' ? '选择书籍语言' : 'Select Book Language'}:</label>
          <select value={bookLang} onChange={e => setBookLang(e.target.value)} className="border rounded px-3 py-2">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {data.map(book => (
              <div key={book.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition transform hover:-translate-y-2">
                ...
                <a href={book.link} target="_blank" className="inline-block px-4 py-2 bg-primary text-white rounded-lg hover:bg-secondary transition">{lang === 'zh' ? '阅读' : 'Read'}</a>
              </div>
            ))}
          </div>
        )}
